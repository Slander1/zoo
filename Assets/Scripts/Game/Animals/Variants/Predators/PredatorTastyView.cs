...
{
public class PredatorTastyView : MonoBehaviour, IDisposable
{
    ...
    {
        StartShowSequenceAsync().Forget();
    }

    private async UniTask StartShowSequenceAsync()
    {
    ...
    private async UniTask HideAfterDelayAsync(float seconds, CancellationToken tcs)
    {
        try
        {
            await UniTask.Delay(
